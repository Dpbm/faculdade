...
class CancelAll(Command):
    ...
    def execute(self):
        self._previous_statuses = self.queue.list_status()
        self.queue.cancel_jobs()

    def undo(self):
        self.queue.update_status(self._previous_statuses)
...
class Queue:
    ...
    def update_status(self, status_list):
        for job,status in zip(self._jobs, status_list):
...
if __name__ == "__main__":
    ...
    history.execute(CancelAll(queue))
    queue.show()
